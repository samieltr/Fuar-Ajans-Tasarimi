...
$sorgu = $baglanti->prepare("SELECT * FROM bilgial");
$sorgu->execute();
$yon = 'sag';
...
while ($sonuc = $sorgu->fetch()) {
    ...
    <tr <?php echo $yon == 'sag' ? 'ml-auto' : 'mr-auto' ?>>
      <td><?= $sonuc['id'] ?></td>
      <td><?= $sonuc['adsoyad1'] ?></td>
	  <td><?= $sonuc['tel1'] ?></td>
	<td>  
	<a href="inc/silbilgi.php?id=<?= $sonuc["id"] ?>" class="btn btn-danger">Sil</a></td>
